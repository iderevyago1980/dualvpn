...
- **Общая lab-сеть, не QEMU-NAT.** `config.Tunnel.Endpoint` содержит только хост
  (клиент всегда идёт на :443), поэтому оба ocserv держим на **443 с разными IP**
  (10.20.0.11 / 10.20.0.12). Клиентские VM подключаются через `tap`-интерфейс к
  Linux-мосту `br-dualvpn`, к которому подключена и docker lab-сеть. Так серверы
  видны на нативном 443 по разным IP — как в реальном мире. Порт в конфиг **не
  добавляем** (если бэкенду нужен non-443 — это отдельная работа, вне рамок).
  Права на `tap`/bridge берём через `sudo` (passwordless есть).
