...
### 1. AstraLinux
- URL: `vpn2.astralinux.ru` (vpn.astralinux.ru редиректит на vpn2)
- Тип: Cisco ASA, AnyConnect SSL/TLS
- SAML/SSO: НЕТ
- Группы (tunnel-groups):
  - `Basic 2FA` → отображается как "Astra2FA" (по умолчанию)
  - `Astra2FA Partners` → "Astra2FA Partners"
  - `Basic` → "AstraLinux"
  - `Partners Astralinux` → "Partners"
  - `AstraLinuxExt` → "Sevastopol"
- 2FA: TOTP (6 цифр) для групп с "2FA" в названии
- CSRF token в форме логина

### 2. MT-Integration
- URL: `vpn.mt-integration.ru`
- Тип: Cisco ASA, AnyConnect SSL/TLS
- SAML/SSO: НЕТ
- Группы:
  - `MT-I_RA` → "MT Remote Access" (по умолчанию)
  - `MT-I_RA_MFA` → "MT Remote Access Multifactor"
  - `MT-I_RA_no_split` → "MT Remote Access full"
- 2FA: TOTP для группы MT-I_RA_MFA
- CSRF token в форме логина
...
Тёмная тема, layout:
- **Слева**: сайдбар со списком туннелей (AstraLinux, MT-Integration) + кнопка "Добавить туннель" + раздел "Глобальные" (Настройки, Статистика, Логи)
