...
//
// Режим "tun" требует админ-прав (создание TUN-интерфейса и маршрутов через
// internal/tun + internal/routing), режим "socks5" работает от обычного
// пользователя (SOCKS5-мост поверх gVisor netstack, без TUN-драйвера).
package mode
...
const (
	TUN    = "tun"    // Полноценный TUN-интерфейс + split-маршруты (нужен админ)
	SOCKS5 = "socks5" // SOCKS5-прокси поверх gVisor netstack (админ не нужен)
)
