...
func TestDualTunnelsSimultaneous(t *testing.T) {
	vpn1 := newGateway(t, "vpn1", "10.10.0.5", "10.10.0.1", "111111")
	vpn2 := newGateway(t, "vpn2", "10.20.0.5", "10.20.0.1", "222222")
	...
	mgr.AddTunnel(mkCfg("vpn1", vpn1))
	mgr.AddTunnel(mkCfg("vpn2", vpn2))
	...
	codes := map[string]string{"vpn1": "111111", "vpn2": "222222"}
	pending := map[string]bool{"vpn1": true, "vpn2": true}
	...
	for _, g := range []*gateway{vpn1, vpn2} {
	...
	// Кросс-проверка изоляции: SOCKS5-порт vpn1 не должен доставать
	// внутреннюю сеть vpn2 — маршруты и netstack у туннелей разные.
	vpn1Proxy := net.JoinHostPort("127.0.0.1", itoa(vpn1.socksPort))
	if conn := tryConnect(vpn1Proxy, vpn2.srv.HostIP(), vpn2.echoPort); conn != nil {
		conn.Close()
		t.Error("через SOCKS5 vpn1 удалось достучаться до внутренней сети vpn2 — туннели не изолированы")
	}
}
